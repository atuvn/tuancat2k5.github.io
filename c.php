...
if(isset($_GET["key"])){
	$key = $_GET["key"];
	if($key == "test" || $key == "key1" || $key == "key2" || $key == "key3" || $check["status"] == "success"){/*
	}if($key == "tungduongit" or $check["status"] == "success"){*/
		if(isset($_GET["phone"])){
			$sdt = $_GET["phone"];
		...
		echo ECHOJSON(array("status" => "error", "msg" => "key sai rồi bạn ơi mua key đi")); exit;
	}
} else {
...
if(strlen($sdt) < 10){
	...
	echo ECHOJSON(array("status" => "error","msg" => "Vui Lòng Nhập Đúng Số Phone")); exit;
} else if($sdt == "0562736654" or $sdt =="0588024068" or $sdt == "0562736654"){
...
"6" => $LOSHIP["LOSHIP"],
"7" => $ZALOPAY["ZALOPAY"],
"8" => $CAYDENTHAN["DENTHAN"],
"9" => $MONEYVEO["MONEYVEO"],
"10" => $TIENOI["TIENOI"],
"11" => $ONCREDIT["ONCREDIT"],
"12" => $DAIHOCFPT["DAIHOCFPT"],
"13" => $VIETTELL["VIETTEL"],
"14" => $VETTELL2["VT.VN"],
"15" => $CAFELAND["CAFELAND"],
"16" => $TAMO["TAMO"],
"17" => $VAYSIEUDE["VAYSIEUDE"],
"18" => $VAYVND["VAYVND"],
"19" => $MCREDIT["MCREDIT"],
"20" => $TUOITRE["TT"],
"21" => $FINDO["FINDO"],
"22" => $DONGPLUS["DONGPLUS"],
"23" => $POPS["POPS"],
"24" => $THANTAIOI["THANTAIOI"],
"25" => $VIEON["VIEON"],
"26" => $AHAMOVE["AHAMOVE"],
"27" => $MONEYDONG["MONEYDONG"],
"28" => $FUNRING["FUNRING"],
"29" => $WINMART["WINMART"],
"30" => $OLDFACEBOOK["OLDFACEBOOK"],
"31" => $VAMO["VAMO"],
"32" => $VIETID["VIETID"],
"33" => $GOTADI["GOTADI"],
"34" => $CONCUNG["CONCUNG"],
"35" => $UBOFOOD["UBOFOOD"],
"36" => $VIETLOTT["VIETLOTT"],
"37" => $PHUCLONG["PHUCLONG"],
"38" => $GAPO["GAPO"],
"39" => $SWIFT247["SWIFT247"],
"40" => $BIBABO["BIBABO"]
